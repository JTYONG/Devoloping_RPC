...
for (double y = gasStart; y <= gasEnd; y += 0.02) {
  ...
  aval.AddElectron(0., y0, 0., 0., 0.1);
  aval.AddElectron(0., y0-0.01, 0., 0., 0.1);
  aval.AddElectron(0., y0-0.02, 0., 0., 0.1);
  aval.AddElectron(0., y0-0.03, 0., 0., 0.1);
  aval.AddElectron(0., y0-0.04, 0., 0., 0.1);
  aval.AddElectron(0., y0-0.05, 0., 0., 0.1);
  LOG("# of electron to be avalanched : " << aval.GetElectrons().size());
...
LOG("Min signal            : " << minSignal << " fC/ns");
  ...
  // 11. PLOT
  // ──────────────────────────────────────────────────
  sensor.ExportSignal(electrode, "ElectronCurrent",true);
  ...
  sensor.ExportSignal(electrode, "ElectronCharge",true);
