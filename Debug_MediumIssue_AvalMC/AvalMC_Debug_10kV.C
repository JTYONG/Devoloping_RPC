...
int main(int argc, char *argv[]) {
  ...
  constexpr bool plotField = true;
   ...
   static constexpr double fGasGapThickness = 0.2;         // cm - total gas gap thickness            2000 micron = 2 mm = 0.2 cm
    ...
    static constexpr double fAnodeVoltage = 5000.0;               // V - ANODE at +1.1kV
    static constexpr double fCathodeVoltage = -5000.0;            // V - CATHODE at -1.1V
  ...
  // Materials needed.
    ...
    const std::string label = "BottomReadout";
    ...
    Box_TopGraphite->SetBoundaryPotential(fCathodeVoltage); // Anode
    ...
    Box_BottomGraphite->SetBoundaryPotential(fAnodeVoltage); // Cathode
  ...
  for (const auto &cluster : track.GetClusters()) {
    ...
    for (const auto &electron : cluster.electrons) {
      ...
      for (const auto& electron : aval.GetElectrons()){
        ...
        avalMCin.DriftIon(p0.x,p0.y,p0.z,p0.t);
        ++nIonMC;
      }
      //avalMCin.AvalancheHole(electron.x,electron.y,electron.z,electron.t); 	      
...
*/
  std::cout<< "Number of Ion Drifts: " << nIonMC<<std::endl;
